...
+ (BOOL)isValidPhone:(NSString *)value
{
    ...
    if (![PPIdentityTool isValidNumber:value]) {
        return FALSE;
    }
...
+ (BOOL)isValidIdentifier:(NSString *)value
{
    ...
    if (15 != length && LENGTH != length) {
        ...
        if (result && length == LENGTH) {
            ...
            if (ids[17] == checktable[checksum % 11] || (str[17] == 'x' && checktable[checksum % 11] == 10)) {
                result = TRUE;
            }
            else {
                result = FALSE;
...
+ (BOOL)isValidCreditNumber:(NSString *)value
{
    ...
    if (length >= 13) {
        result = [PPIdentityTool isValidNumber:value];
        if (result) {
            ...
            // Diner's Club
            if (((threeDigitBeginValue >= 300 && threeDigitBeginValue <= 305) || fourDigitBeginValue == 3095 || twoDigitBeginValue == 36 || twoDigitBeginValue == 38) && (14 != length)) {
                result = FALSE;
            }
            // VISA
            else if ([[value substringToIndex:1] isEqualToString:@"4"] && !(13 == length || 16 == length)) {
                result = FALSE;
            }
            // MasterCard
            ...
            // American Express
            else if (([[value substringToIndex:2] isEqualToString:@"34"] || [[value substringToIndex:2] isEqualToString:@"37"]) && (15 != length)) {
                result = FALSE;
            }
            // Discover
            else if ([[value substringToIndex:4] isEqualToString:@"6011"] && (16 != length)) {
                result = FALSE;
            }
            else {
...
+ (BOOL)isValid:(IdentifierType)type value:(NSString *)value
{
    ...
    switch (type) {
        case IdentifierTypeZipCode:
            result = [PPIdentityTool isValidZipcode:value];
            break;
        case IdentifierTypeEmail:
            //            result = [IdentifierValidator isValidEmail:value];
            result = [PPIdentityTool validateEmail:value];
            break;
        case IdentifierTypePhone:
            result = [PPIdentityTool isValidPhone:value];
            break;
        case IdentifierTypeUnicomPhone:
            result = [PPIdentityTool isChinaUnicomPhoneNumber:value];
            break;
        ...
        case IdentifierTypeString:
            result = [PPIdentityTool isValidString:value];
            break;
        case IdentifierTypeIdentifier:
            result = [PPIdentityTool isValidIdentifier:value];
            break;
        case IdentifierTypePassort:
            result = [PPIdentityTool isValidPassport:value];
            break;
        case IdentifierTypeCreditNumber:
            result = [PPIdentityTool isValidCreditNumber:value];
            break;
        case IdentifierTypeBirthday:
            result = [PPIdentityTool isValidBirthday:value];
            break;
        default:
            break;
    }
